...
let joined_pattern_type = (ctx, rules) => {
  ...
  Construction.HTyp.join_all(ctx, LUB, tys);
};
    ...
    let correct_rule_types =
      List.for_all2(
        (rule_ty, rule) => {
          ...
          | None => false
          | Some(syn_ty) => Construction.HTyp.equiv(ctx, rule_ty, syn_ty)
          }
        },
        rule_types,
        rules,
...
and syn_rules =
  ...
  Construction.HTyp.join_all(ctx, GLB, clause_types);
}
and syn_rule =
...
and ana_skel =
  ...
  | BinOp(
    ...
    Construction.HTyp.consistent(ctx, ty, ty') ? Some() : None;
  }
and ana_operand =
...
and syn_fix_holes_line =
  ...
  | TyAliasLine(p, ty) =>
    let (ty, kind, u_gen) = Elaborator_Typ.syn_fix_holes(ctx, u_gen, ty);
    let (body_ctx, p) = Statics_TPat.fix_holes(ctx, p, kind);
...
and syn_fix_holes_rules =
  ...
  let common_type = Construction.HTyp.join_all(ctx, GLB, rule_types);
